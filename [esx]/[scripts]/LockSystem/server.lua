local vehStorage = {}
local randomMsg = {	
                     "Vous avez trouve les clef sur le pare-soleil",
                     "Vous avez trouve les clef dans la boîte a gants.",
                     "Vous avez trouve les clef sur le siege du passager.",
                     "Vous avez trouve les clef sur le sol.",
                     "Les cles etaient deja sur le contact, vous les avez prises."}
	...
	for i=1, #(vehStorage) do
		if vehStorage[i].plate == plate then
			...
			if vehStorage[i].owner == playerIdentifier then
				...
				TriggerClientEvent("ls:sendNotification", source, notifParam, "Vous n'avez pas la clef de ce véhicule.", 0.5)
				break
			end
		end
	end
