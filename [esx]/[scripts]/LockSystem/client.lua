...
AddEventHandler("ls:lock", function(lockStatus, vehicle)
	...
	if lockStatus == 1 or lockStatus == 0 then -- Si le véhicule est déverrouillé (on le verrouille):
		...
		-- ## Notifications
			...
			TriggerEvent("ls:sendNotification", notificationParam, "Vehicule Fermer.", 0.080)
		-- ## Notifications
	...
	elseif lockStatus == 2 then -- Si le véhicule est verrouillé
		...
		-- ## Notifications
			...
			TriggerEvent("ls:sendNotification", notificationParam, "Vehicule Ouvert.", 0.080)
		-- ## Notifications
...
AddEventHandler("ls:notify", function(text, time)
	...
	Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_LIFEINVADER", "CHAR_LIFEINVADER", true, 1, "Clef Voiture", "Version 3.0.0", time)
...
AddEventHandler("ls:sendNotification", function(param, message, duration)
	...
	elseif param == 2 then
		TriggerEvent('chatMessage', 'ClefVoiture', { 255, 128, 0 }, message)
	end
end)
