		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Quitter votre vehicule")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Appuyer sur ~g~G ~w~annuler le ravitaillement de votre véhicule. $~r~" .. price .. " ~w~+  tax")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Le véhicule est trop rempli d'essence pour être alimenté")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Vous n’avez actuellement pas assez d’argent pour acheter du carburant avec")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Appuyer sur ~g~G ~w~pour faire le plein de votre véhicule. $~r~0.5/~w~gallon + tax")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Appuyer sur ~g~G ~w~pour stoper le plein. Currently at: " .. fuel .. "% - Jerry Can: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Apuyyer ~g~G ~w~alimenter le véhicule avec votre bidon d'essence")
...
function DisplayHud()
	...
	--	local kmh 	  =	round(GetEntitySpeed(vehicle) * 3.6, 0)
		...
		DrawAdvancedText(0.1720 - x, 0.7765 - y, 0.005, - 0.0320, 0.5, fuel, 255, 255, 255, 255, 6, 1)

		--DrawAdvancedText(0.130 - x, 0.77 - y, 0.005, 0.0028, 0.6, mph, 255, 255, 255, 255, 6, 1)
		--DrawAdvancedText(0.174 - x, 0.77 - y, 0.005, 0.0028, 0.6, kmh, 255, 255, 255, 255, 6, 1)

		DrawAdvancedText(0.1200 - x, 0.7765 - y, 0.005, - 0.0320, 0.5, "Carburant  :", 255, 255, 255, 255, 6, 1)
	end
end
